...
def calculate_section_averages(data):
    ...
    for section_data in data:
        ...
        labels.append(section_name.split('. ')[1])
        scores.append(avg)
...
for section_data in checklist_data:
    ...
    with st.expander(f"**{section_data['section']}**{current_avg_str}", expanded=False):
        for item in section_data["items"]:
            ...
            with col2:
                # MODIFICA QUI: Aumentata altezza minima
                st.text_area(
                    "Note:",
                    key=item["key_notes"],
                    value=st.session_state[item["key_notes"]],
                    height=70, # Aumentato da 50 a 70 (minimo 68 richiesto)
                    label_visibility="collapsed"
                )
            st.markdown("---")
...
if not section_labels or not any(s > 0 for s in section_scores): # Controlla se ci sono punteggi > 0
    st.warning("Inserisci almeno un punteggio per generare i grafici.")
else:
    # 1. Grafico Radar
    fig_radar = go.Figure()
    valid_scores_radar = [s if s > 0 else 0.1 for s in section_scores] # Evita 0 per visibilità radar
